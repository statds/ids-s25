...
# Convert ZIP codes to strings, but handle missing values separately
df['zip_code'] = df['zip_code'].apply(lambda x: str(int(x)).zfill(5) if pd.notnull(x) else np.nan)
...
invalid_zips = df[df['zip_code_numeric'].isna() | ~df['zip_code_numeric'].isin(all_valid_zips)]['zip_code']
