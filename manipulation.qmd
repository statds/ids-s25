...
invalid_zips = df[
    df['zip_code_numeric'].isna() | ~df['zip_code_numeric'].isin(all_valid_zips)
    ]['zip_code']
...
with open("censusAPIkey.txt") as f:
...
of 2023 for all the NYC zip codes.
```{python}
ACS_YEAR = 2023
...
```{python}
acs_data = c.acs5.get(
    list(ACS_VARIABLES.keys()), {'for': 'zip code tabulation area:*'}
)
...
df_acs = df_acs[df_acs["zip code tabulation area"].isin(all_valid_zips)]
...
gdf['Land Area (sq miles)'] = gdf['geometry'].area / 2_589_988.11
# 1 square mile = 2,589,988.11 square meters

print(gdf[['modzcta', 'Land Area (sq miles)']].head())
```
...
gdf['Population Density (per sq mile)'] = (
    gdf['Total Population'] / gdf['Land Area (sq miles)']
    )

# Display first few rows
print(gdf[['modzcta', 'Total Population', 'Land Area (sq miles)',
    'Population Density (per sq mile)']].head())
```
...
gdf.plot(column='Population Density (per sq mile)', 
...
# Add a title
ax.set_title("Population Density by ZIP Code in NYC", fontsize=14)
...
Let's save the merged data for future usage.
```{python}
csv_output = "data/NYC_Zipcode_Data.csv"

gdf[['modzcta', 'Total Population', 'Land Area (sq miles)',
    'Population Density (per sq mile)']].to_csv(csv_output, index=False)
