## A Grammar of Graphics Primer with Plotnine
...
# Remove rows where borough is NaN, and make a clean copy
...
df_borough = (
    df.groupby(["borough", "day_of_week"])
      .agg({"number_of_persons_injured": "sum"})
      .reset_index()
)
...
df_borough["day_of_week"] = pd.Categorical(
    df_borough["day_of_week"],
    categories=day_order,
    ordered=True
)

# Extract crash hour from various formats like "23", "07", or "23:15"
...
# Classify each crash as Day or Night
df["time_of_day"] = df["crash_hour"].apply(
    lambda h: "Day" if pd.notnull(h) and 6 <= h < 18 else "Night"
)
