...
1. Initialization: 
   ...
   - For classification: 
   $F_0(x) = \log \left( \frac{\Pr(y = 1)}{1 - \Pr(y = 1)} \right)$.
...
1. _Loss Function_: The loss function measures the discrepancy 
   between the actual and predicted values. It guides the model 
   updates. Common choices include:
   
   - Squared error for regression: 
     $L(y, F(x)) = \frac{1}{2} (y - F(x))^2$. Pseudo-residuals are
     exactly residuals.
   - Logistic loss for binary classification: Log likelihood of
     Bernoulli data.

2. _Learning Rate_: The learning rate scales the contribution of 
...
3. _Regularization_: Boosted trees incorporate regularization to 
...
- XGBoost: Known for its speed and performance, with features like 
  regularization, random subsampling, and advanced tree pruning.
